...
export class ArchiviodocumentiPage implements OnInit {
  ...
  estrazioneFileFolder(event = null) {
    this.gs
      .callGateway(
        '0i2+RmInGMX0DvOaBMb2+YgiKEYyPyb1UckrhmUoRUotWy0tSVYtWy0xLsiHvQYzJJ7lAyA1aGtvSXKul9MHKkoycGAVlT+pvg@@',
        `${this.ricerca.folderid},@foldername,@parentid,@parentname`,
      )
      .subscribe(
        (data) => {
  ...
  nuovoFile() {
    const modalCliente = this._modal.present(ModalCaricaFileComponent, {
      folderid: this.ricerca.folderid,
    });
  ...
  nuovoCartella() {
    ...
    modalNew.then((result) => {
      if (result.data && result.data.descrizione) {
        ...
        this.gs
          .callGateway(
            'dNJI9LXEVkvhVA0PaJtwD6L1q6tfA1ZXsaZMJyuxlAUtWy0tSVYtWy1nspxg4PgQPJXIYQsxTVgBw9NiqJFN6hL09UIyi0ejrQ@@',
            `'${descrizione}',${this.ricerca.folderid}`,
          )
          .subscribe(
            (data) => {
              if (data.hasOwnProperty('error')) {
  ...
  deleteFolder($event, fileFolder: ArchivioFile) {
    ...
    const alertElimina = this._alert.confirm(
      'Attenzione',
      `Confermi di eliminare della cartella "${fileFolder.descrizione}" e TUTTO quello che contiene ?`,
    );
    alertElimina.then((result) => {
      if (result.role === 'OK') {
        this.gs
          .callGateway(
            '2ymowsy6h5RjmGLLwyCnmec4Xg5+Q3awNSSt6FXHz0ctWy0tSVYtWy3STQS3Onw2HQ6AGQvD8UN22C/Fwfi5L0YoEcLLJhH7+Q@@',
            `${fileFolder.folderid}`,
          )
          .subscribe(
            (data) => {
  ...
  deleteFile($event, fileFolder: ArchivioFile) {
    ...
    const alertElimina = this._alert.confirm(
      'Attenzione',
      `Confermi di eliminare il file "${fileFolder.descrizione}" ?`,
    );
    alertElimina.then((result) => {
      if (result.role === 'OK') {
        this.gs
          .callGateway(
            '2H6fgL8RL3pOrFGxXCopTYpWYOhLkBlwYhnnhWceTe4tWy0tSVYtWy1fTzDMAyCc1Um89Bg3UOXRiuG1dWW+NMqxJksng9UkOg@@',
            fileFolder.arc_codi,
          )
          .subscribe(
            (data) => {
              ...
              this._ds.delete({ mode: 3, path: fileFolder.id_storage }).subscribe(
                (data) => {},
                (error) => this.gs.toast.present(error.message),
              );
              this.estrazioneFileFolder();
  ...
  move($event, file: ArchivioFile) {
    ...
    modalFolder.then((result) => {
      if (result.data) {
        this.gs
          .callGateway(
            'wVp8CePj9GWqbn4+9PSPrfcnBgJy5X6770MUTmuu8VEtWy0tSVYtWy2811ekJtlErGybjvOQXCisQNJvW7NZXC65kgNNldAyog@@',
            `${file.arc_codi},${result.data.id}`,
          )
          .subscribe(
            (data) => {
              if (data.hasOwnProperty('error')) {
  ...
  updateFolderName(folder: ArchivioFile) {
    this.gs
      .callGateway(
        'EO8JC8YQlb1qMkXVspBpN0TSg51g6MJE8AgoukLdy0stWy0tSVYtWy2+vCVIYV+sszqVEKfxn6c9KFZM1vtSrQBhvXMBKxU5iw@@',
        `${folder.folderid},'${folder.descrizione}'`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  download($event, file: any) {
    $event.stopPropagation();
    this._ds.get({ mode: 4, path: file.id_storage }).subscribe(
      (file) => {
