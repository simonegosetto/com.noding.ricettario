...
export class SchedeproduzionePage implements OnInit {
  ...
  list(event = null) {
    this.gs
      .callGateway(
        'JF8yXz5TOp18Sotb5VMkfjvUv8HmSOtIm4S02S8XjQ4tWy0tSVYtWy1YFqLxfMIhA3RMGauQYR2m5e5tPNHBFW8t1VyFVpA8lg@@',
        ``,
      )
      .subscribe(
        (data) => {
  ...
  private _saveListino(scheda: SchedeProduzione) {
    this.gs
      .callGateway(
        'xHdQPwuFw1H1KO/cRHQlPrTaWgxmNT3nS5b9YaOBlW4tWy0tSVYtWy04WGL10k1ciZBMp7DlMdlugRxaidsNbmKfPeBzmzmpxQ@@',
        `${scheda.id},'${scheda.descrizione}'`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  delete($event, scheda: SchedeProduzioneRead) {
    ...
    const alertElimina = this._alert.confirm(
      'Attenzione',
      `Confermi di eliminare il listino ${scheda.descrizione} ?`,
    );
    alertElimina.then((result) => {
      if (result.role === 'OK') {
        this.gs
          .callGateway(
            'b6MZgUoZ3Wk49DsGooqOlsxO+MzfsReH/y0uLX63xF4tWy0tSVYtWy33pNRXlrm4nwQ988rgLtrOLEvfmvS36E+aklKoi80BPA@@',
            scheda.id,
          )
          .subscribe(
            (data) => {
