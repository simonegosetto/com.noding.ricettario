...
export class MenusPage implements OnInit {
    ...
    private _save(listino: Menu) {
        if (listino.id > 0) {
            this.gs.callGateway('WddXaeNoWWXfUYNDtmREohVHFM/7T/G7ffefI3q3RE8tWy0tSVYtWy3QneYethYCTEyg74Y9QyTQBX0Nt0lqHoTn12NCP6P1og@@',
                `${listino.id},'${listino.descrizione}',${listino.tipo}`).subscribe(data => {
                    if (data.hasOwnProperty('error')) {
                        this.gs.toast.present(data.error);
                        return;
                    }
