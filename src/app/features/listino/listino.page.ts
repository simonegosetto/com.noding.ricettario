...
export class ListinoPage implements OnInit {
    ...
    private _saveMenu(menuID: number) {
        this.gs.callGateway('QYJrZrRzOMFO3XpMeU0AeUWJZrkcEqIpC0nohJJeHBotWy0tSVYtWy1dOzu+Lbh7TikceENNj8rI2N6VEbINKQOJyCJc6k2FSg@@',
            `${menuID},${this.listino.id}`).subscribe(data => {
                if (data.hasOwnProperty('error')) {
                    this.gs.toast.present(data.error);
                    return;
                }
