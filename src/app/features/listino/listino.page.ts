...
import { ModalSearchMenuListinoComponent } from './modal-search-menu-listino.component';
...
export class ListinoPage implements OnInit {
  ...
  private _estrazioneCategorie() {
    this.gs
      .callGateway(
        'gKAvUn6c5bcp110Jh/vcbsnEbwctvJY0zl6gC0K3qa4tWy0tSVYtWy1H1JD+sBUdce4kGbfi8PFOJ6glv1neuYU/19MzXKDDSQ@@',
        ``,
      )
      .subscribe(
        (data) => {
  ...
  getListinoRighe(event = null) {
    this.gs
      .callGateway(
        'j8nnE18DvMJLJn72/pLhFHuvrOA97O1D70EeY0K28/otWy0tSVYtWy1k5ipImnHJdjEgS2084WWHruF1SwXI5uZouBYSWHPGGQ@@',
        `${this.listino.id},${this.ricerca.categoria}`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  print() {
    window.open(
      environment.apiReportListino +
        '?params=' +
        this.listino.id +
        '&categoria=' +
        this.ricerca.categoria +
        '&token=' +
  ...
  edit($event, riga: ListinoRiga) {
    ...
    const modalEditIngrediente = this._modal.present(ModalEditIngredienteListinoComponent, {
      ...riga,
  ...
  delete($event, riga: ListinoRiga) {
    ...
    const alertElimina = this._alert.confirm(
      'Attenzione',
      `Confermi di eliminare l'ingrediente ${riga.descrizione} ?`,
    );
    alertElimina.then((result) => {
      if (result.role === 'OK') {
        this.gs
          .callGateway(
            'OemnSwfiRPzAVbNM8+3RBZ4O4zVb5T9yN53H7Ax02YwtWy0tSVYtWy0lxrGjs502JyZQihu3oKcFrDNuTa+3V95N9HRsztRp2A@@',
            riga.id,
          )
          .subscribe(
            (data) => {
  ...
  nuovoFromMenu() {
    const modalNewFromMenu = this._modal.present(ModalSearchMenuListinoComponent, {});
    modalNewFromMenu.then((result) => {
      if (result.data) {
        this._saveMenu(result.data);
      }
    });
  }

  private _saveMenu(menuID: number) {
    this.gs
      .callGateway(
        'QYJrZrRzOMFO3XpMeU0AeUWJZrkcEqIpC0nohJJeHBotWy0tSVYtWy1dOzu+Lbh7TikceENNj8rI2N6VEbINKQOJyCJc6k2FSg@@',
        `${menuID},${this.listino.id}`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  private _saveSchedaTecnica(schedaTecnicaID: number) {
    this.gs
      .callGateway(
        'M+znAuYqN5Ug1l2iGVkxduf1lbRlT1UuSoxLW1oid8stWy0tSVYtWy34EKDV2l7UJrjG/oWkhSl8rBxmw+GSDbr4MKJeU8pXfA@@',
        `${this.listino.id},${schedaTecnicaID}`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  private _saveIngrediente(ingredienteID: number) {
    this.gs
      .callGateway(
        'E+pKhhlKoT3QoiETImqtg7Tm6Zt2Hw828idhcpov3FeYPL2p7Y+IYRhDaAM1DR/OZYyzo/TQq8WD17k93BrohS1bLS1JVi1bLddOB+8ied/iwzy2sftWM7NH2B+A3DlBQJmKEMJu0DNp',
        `${this.listino.id},${ingredienteID}`,
      )
      .subscribe(
        (data) => {
          if (data.hasOwnProperty('error')) {
  ...
  private _updateIngrediente(ingrediente: IngredienteListino) {
    this.gs
      .callGateway(
        '/nGnbEpkUi0vOg4i6e1iuEGuxg69lsTmVCr8YyLq268tWy0tSVYtWy3L2b8m+30DhYzDhrNYxDVYk6RvH3uhYOxox9hDnFFDyQ@@',
